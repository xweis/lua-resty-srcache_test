...
function _M.urlCheck(self,url)
    ...
    for k,v in pairs(cache_url_list) do
        if url == k then
            return v
        end
    end
...
function _M.run(self)
    ...
    local aexptime = self.urlCheck(self, ngx.re.sub(ngx.var.request_uri, "\\?.*", ""))
    if not aexptime or aexptime < 0 then
        ngx.log(NOTICE, aexptime)
        ngx.var.is_cache = 0
        return
    end
